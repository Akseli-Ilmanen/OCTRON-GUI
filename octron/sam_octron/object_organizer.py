...
class Obj(BaseModel):
    ...
    annotation_layer: Any | None = None
...
class ObjectOrganizer(BaseModel):
    ...
    >>> object_organizer.add_entry(
    ...     1, Obj(label="worm", suffix="1")
    ...
    def all_colors(self) -> tuple[list, list, list]:
        ...
        indices_max_diff_labels = sample_maximally_different(
            list(range(self.n_labels_max))
        )
        indices_max_diff_subcolors = sample_maximally_different(
            list(range(self.n_subcolors))
        )
        return (
            label_colors,
    ...
    def exists_combination(self, label: str, suffix: str) -> bool:
        ...
        return any(
            entry.label == label and entry.suffix == suffix
    ...
    def get_entries_by_label(self, label: str) -> list[Obj]:
        ...
        return [
            entry for entry in self.entries.values() if entry.label == label
        ]
    ...
    def update_entry(self, id_: int, entry: Obj) -> None:
        ...
        for eid, existing in self.entries.items():
            if (
                eid != id_
                and existing.label == entry.label
    ...
    def get_entry_id(self, organizer_entry: Obj) -> int | None:
    ...
    def get_entry(self, id_: int) -> Obj | None:
    ...
    def save_to_disk(self, file_path: str | Path):
        ...
        for obj_id, obj in self.entries.items():
            ...
            serializable_obj = obj.model_dump(
                exclude={"annotation_layer", "prediction_layer"}
            )
            ...
            if obj.prediction_layer is not None:
                ...
                serializable_obj["prediction_layer_metadata"] = {
                    ...
                    if hasattr(obj.prediction_layer.data, "shape")
                    else None,
                    ...
                    "video_file_path": prediction_layer_meta[
                        "_video_file_path"
                    ].as_posix(),
                ...
                if (
                    hasattr(obj.prediction_layer, "metadata")
                    and "_zarr" in obj.prediction_layer.metadata
                ):
                    serializable_obj["prediction_layer_metadata"][
                        "zarr_path"
        ...
        if file_path.exists():
            logger.warning(
                f"⚠️ Overwriting existing file at {file_path.as_posix()}"
            )
            file_path.unlink()
        with open(file_path, "w") as f:
            json.dump(serializable_data, f, indent=2)
        logger.info(
            f"💾 Octron object organizer saved to {file_path.as_posix()}"
        )
