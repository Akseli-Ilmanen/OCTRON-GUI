...
class sam_octron_callbacks:
            ...
            if shapes_layer.mode == "add_rectangle":
                ...
                try:
                    shape_masks = np.stack(
                        shapes_layer.to_masks((video_height, video_width))
                    )
                    if len(shape_masks) == 1:
                        shape_mask = shape_masks[0]
                    else:
                        frame_indices = np.array(
                            [s[0][0] for s in shapes_layer.data]
                        ).astype(int)
    ...
    def _handle_semantic_box_detection(
        ...
        prediction_layer.colormap = create_semantic_colormap(
            n_objects, label_id=organizer_entry.label_id
        )
        prediction_layer.refresh()
        ...
        if hasattr(prediction_layer.data, "attrs"):
            ...
            prediction_layer.data.attrs["max_object_id"] = max(
                int(n_objects), int(prev_max)
            )
    ...
    def feed_rectangles_to_predictor(self):
        ...
        if not isinstance(predictor, SAM3_semantic_octron):
            show_warning(
                "This function is only available in SAM3 semantic mode."
            )
            return
        if not self.octron.prefetcher_worker:
            show_warning("Prefetcher worker not initialized.")
        ...
        for obj_id, entry in self.octron.object_organizer.entries.items():
            ...
            for shape_data in shapes_layer.data:
                ...
                boxes_xyxy.append(
                    [float(tl[1]), float(tl[0]), float(br[1]), float(br[0])]
                )
    ...
    def on_points_changed(self, event):
        ...
        if action == "added":
            ...
            if self.left_right_click == "left":
                ...
                points_layer.border_color[-1] = [0.7, 0.7, 0.7, 1]
                points_layer.symbol[-1] = "o"
            elif self.left_right_click == "right":
                label = 0
                points_layer.face_color[-1] = [0.7, 0.7, 0.7, 1]
    ...
    def prefetch_images(self):
        ...
        end_frame = min(
            num_frames - 1, current_frame + chunk_size * skip_frames
        )
        ...
        if (
            ...
            if hasattr(tracker, "_prefetch_backbone_batch"):
                tracker._prefetch_backbone_batch(
                    prefetch_indices, batch_size=4
                )
            elif hasattr(tracker, "_get_image_feature"):
                with torch.inference_mode():
                    for idx in prefetch_indices:
                        tracker._get_image_feature(
                            tracker.inference_state, idx, batch_size=1
                        )
    ...
    def next_predict(self):
        ...
        end_frame = min(
            num_frames - 1,
            current_frame + self.octron.chunk_size * skip_frames,
        )
