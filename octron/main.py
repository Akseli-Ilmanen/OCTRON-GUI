...
)  # Important for example for .svg files
...
class octron_widget(QWidget):
    ...
    def __init__(self, viewer: "napari.viewer.Viewer", parent=None):
        ...
        self.object_organizer = (
            ObjectOrganizer()
        )  # Initialize top level object organizer
        ...
        self.sam2models_dict = check_sam2_models(
            SAM2p1_BASE_URL="",
        ...
        self.sam3models_dict = check_sam3_models(
            models_yaml_path=sam3models_yaml_path,
            force_download=False,
        )
        ...
        for model_id, model in self.sam2models_dict.items():
            ...
            if "tooltip" in model:
                ...
                self.sam_model_list.setItemData(
        ...
        for tracker in self.trackers_dict:
            if self.trackers_dict[tracker]["available"]:
                logger.info(
                    f"Adding tracker {self.trackers_dict[tracker]['name']}"
                )
    ...
    def gui_callback_functions(self):
        ...
        self.predict_next_batch_btn.clicked.connect(
            self.init_prediction_threaded
        )
        self.predict_next_oneframe_btn.clicked.connect(
            self.init_prediction_threaded
        )
        self.create_projection_layer_btn.clicked.connect(
            self.create_annotation_projections
        )
        self.annotation_jump_previous_btn.clicked.connect(
            self.jump_to_previous_annotated_frame
        )
        self.annotation_jump_next_btn.clicked.connect(
        ...
        self.train_generate_groupbox.setTitle(
            "Generate training data (Mode: Segmentation)"
        )
        ...
        self.segmentation_radiobutton.toggled.connect(
            self.on_train_mode_changed
        )
    ...
    def on_train_mode_changed(self, checked):
        ...
        if checked:
            ...
            self.train_generate_groupbox.setTitle(
                "Generate training data (Mode: Detection)"
            )
    ...
    def _check_model_data_compatibility(self, model_name):
        ...
        """
        if (
            self.project_path_video is None
        ...
        existing_mask_zarrs = list(
            self.project_path_video.glob("*masks*.zarr")
        )
        ...
        is_sam3 = any(
            m["name"] == model_name for m in self.sam3models_dict.values()
        )
    ...
    def load_sam2model(
        ...
        for model_id, model in self.sam2models_dict.items():  # noqa: B007
            if model["name"] == model_name:
                ...
                break
        assert model_found, (
            f"Model '{model_name}' not found in SAM2 models dictionary."
        )
        ...
        checkpoint_path = (
            get_sam_checkpoints_dir() / Path(model["checkpoint_path"]).name
        )
        self.predictor, self.device = build_sam2_octron(
    ...
    def load_sam3model(self, model_name=""):
        ...
        for model_id, model in self.sam3models_dict.items():  # noqa: B007
            if model["name"] == model_name:
                ...
                break
        assert model_found, (
            f"Model '{model_name}' not found in SAM3 models dictionary."
        )
        ...
        if semantic:
            ...
            self.layer_type_combobox.model().item(points_index).setEnabled(
                False
            )
    ...
    def _batch_predict_yielded(self, value):
        ...
        if obj_id in semantic_map:
            ...
            if buf is not None and (
                buf[0] is not prediction_layer or buf[1] != frame_idx
            ):
    ...
    def init_prediction_threaded(self):
        ...
        if isinstance(self.predictor, SAM3_semantic_octron):
            ...
            tracker_needs_rebuild = not self.predictor.inference_state.get(
                "tracking_has_started", False
            ) and any(
                hasattr(e, "_semantic_accumulated_masks")
                and e._semantic_accumulated_masks
            ...
            if tracker_needs_rebuild:
                ...
                for obj_id, entry in self.object_organizer.entries.items():
                    if (
                        hasattr(entry, "_semantic_accumulated_masks")
                        and entry._semantic_accumulated_masks
                    ):
                        entries_with_masks.append((obj_id, entry))
                        all_cond_frames.update(
                            entry._semantic_accumulated_masks.keys()
                        )
                ...
                target_frame = (
                    max(all_cond_frames) if all_cond_frames else None
                )

                for obj_id, entry in entries_with_masks:
                    id_mask = entry._semantic_accumulated_masks.get(
                        target_frame
                    )
                    ...
                    if id_mask is None and target_frame is not None:
                        ...
                        if prediction_layer is not None:
                            zarr_data = np.array(
                                prediction_layer.data[target_frame, :, :]
                            )
                            if zarr_data.max() > 0:
                                id_mask = zarr_data
                    ...
                    if id_mask is None:
                        ...
                        cond_frame = next(
                            iter(entry._semantic_accumulated_masks)
                        )
                    ...
                    for mask_id in unique_ids:
                        ...
                        self._semantic_obj_id_map[tracker_obj_id] = (
                            obj_id,
                            int(mask_id),
                        )
        ...
        valid = False
        if (
            self.predictor.inference_state["point_inputs_per_obj"]
            or self.predictor.inference_state["mask_inputs_per_obj"]
        ):
            valid = True
        if not valid:
        ...
        if sender == self.predict_next_batch_btn:
            self.prediction_worker = create_worker(
                self.sam_octron_callbacks.batch_predict
            )
            ...
            self.prediction_worker_one = create_worker(
                self.sam_octron_callbacks.next_predict
            )
            self.prediction_worker_one.setAutoDelete(True)
            self.prediction_worker_one.yielded.connect(
                self._batch_predict_yielded
            )
            self.prediction_worker_one.finished.connect(
                self._on_prediction_finished
            )
    ...
    def save_object_organizer(self):
        ...
        self.object_organizer.settings = {
            ...
            "sam3_detect_threshold": self.sam3detect_thresh.text().strip()
            or None,
        }
    ...
    def refresh_label_table_list(self, delete_old=False):
        ...
        if not hasattr(self, "label_table_model"):
            ...
            self.existing_data_table.customContextMenuRequested.connect(
                self.on_label_table_context_menu
            )
        ...
        self.generate_training_data_btn.setText("▷ Generate")

        # Enable training tab if data is available
        if label_dict and any(
            v
            for k, v in label_dict.items()
            if k != "video" and k != "video_file_path"
        ):
    ...
    def on_label_table_context_menu(self, pos):
        ...
        action = menu.exec_(
            self.existing_data_table.viewport().mapToGlobal(pos)
        )
        ...
        if action == delete_action:
            ...
            video_file_path = self.label_table_model.label_dict[
                folder_path
            ].get("video_file_path", "")
    ...
    def on_label_table_double_clicked(self, index):
        ...
        video_file_path = Path(
            self.label_table_model.label_dict[folder_path].get(
                "video_file_path"
            )
        )
        ...
        self.load_sam_model_btn.setText("Load model")
        ...
        self.label_list_combobox.addItem("\u2295 Create")
        self.label_list_combobox.addItem("\u2296 Remove")
        ...
        for obj_id, obj_data in object_organizer_data["entries"].items():
            ...
            if "annotation_layer_metadata" in obj_data:
                annotation_type = obj_data["annotation_layer_metadata"].get(
                    "type"
                )
    ...
    def open_project_folder_dialog(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select Base Folder", str(Path.home())
        )
        if folder:
    ...
    def on_layer_removed(self, event):
        ...
        if not self.remove_current_layer:
            ...
            # This has more drastic consequences ...
                ...
                self.layer_to_remove._basename() == "Image"
                and "VIDEO" in self.layer_to_remove.name
            ):
                # What to do:
    ...
    def on_layer_removing(self, event):
        ...
        if self.layer_to_remove._basename() in [
            "Shapes",
            "Points",
            "Image",
            "Labels",
    ...
    def consolidate_layers(self, event):
        ...
        if self.video_layer == video_layer:
            ...
            self.current_video_hash = video_metadata["hash"][
                -8:
            ]  # Save it globally for quick access
            self.project_path_video = (
                self.project_path / self.current_video_hash
            )
    ...
    def on_mp4_file_dropped_area(self, video_paths):
        ...
        if self.video_layer is not None:
            show_warning(
                "A video layer already exists. Please remove it first."
            )
            return
        ...
        video_path = Path(
            video_paths[0]
        )  # Take only the first file if there are multiple
        ...
        video_dict["_name"] = (
            layer_name  # Octron convention. Save the name in metadata.
        )
        layer_dict = {
            "name": layer_name,
            "metadata": video_dict,
        }
        add_layer = self._viewer.add_image
        add_layer(
    ...
    def init_zarr_prefetcher_threaded(self):
        ...
        logger.info(
            f"Resized video dimensions: {resized_height}x{resized_width}"
        )
        ...
        if video_zarr_path.exists():
            ...
            video_zarr, status = load_image_zarr(
                video_zarr_path,
                num_frames=num_frames,
        ...
        if not status or not video_zarr_path.exists():
            ...
            self.video_zarr = create_image_zarr(
                video_zarr_path,
                num_frames=num_frames,
                image_height=resized_height,
                image_width=resized_width,
            ...
            with open(video_info_path, "w") as f:
                ...
                f.write(
                    f"Video abbreviated hash: {self.current_video_hash}\n"
            ...
            logger.info(
                f'Video zarr archive loaded "{video_zarr_path.as_posix()}"'
            )
    ...
    def on_label_change(self):
        ...
        all_list_entries = [
            self.label_list_combobox.itemText(i)
            for i in range(self.label_list_combobox.count())
        ]
        if index == 0:
            return
        elif index == 1:
            ...
            if dialog.result() == QDialog.Accepted:
                ...
                new_label_name = (
                    new_label_name.strip().lower()
                )  # Make sure things are somehow unified
            ...
            if dialog.result() == QDialog.Accepted:
                ...
                self.label_list_combobox.removeItem(
                    self.label_list_combobox.findText(selected_label)
                )
                self.label_list_combobox.setCurrentIndex(0)
    ...
    def create_annotation_layers(
        ...
        if not recreate:
            ...
            if not label or not layer_type:
                ...
                label = label.strip()
                label_suffix = (
                    label_suffix.strip().lower() if label_suffix else ""
                )
        ...
        if organizer_entry is not None:
            if organizer_entry.prediction_layer is None:
                ...
                show_warning(
                    f"Combination ({label}, {label_suffix}) already exists."
                )
                return
            else:
            ...
            status = self.object_organizer.add_entry(
                obj_id, Obj(label=label, suffix=label_suffix)
            )
            if not status:
                show_error("Error when adding new entry to object organizer.")
                return
            organizer_entry = self.object_organizer.entries[
                obj_id
        ...
        if not obj_color:
            obj_color = (
                organizer_entry.color
            )  # Get the color from the organizer entry
        ...
        if create_prediction_layer:
            ...
            mask_colors = DirectLabelColormap(
                color_dict={None: [0.0, 0.0, 0.0, 0.0], 1: obj_color},
                use_selection=True,
                selection=1,
            )
            prediction_layer, zarr_data, zarr_file_path = add_sam_mask_layer(
                viewer=self._viewer,
            ...
            prediction_layer.metadata["_obj_id"] = (
                obj_id  # This corresponds to organizer entry id
            )
            prediction_layer.metadata["_zarr"] = zarr_file_path.relative_to(
                self.project_path
            )
        ...
        if layer_type == "Shapes":
            ...
            annotation_layer = add_sam_shapes_layer(
                viewer=self._viewer,
            ...
            annotation_layer.events.data.connect(
                self.sam_octron_callbacks.on_shapes_changed
            )
            ...
            # Create a shape layer
            annotation_layer = add_sam_points_layer(
                viewer=self._viewer,
                name=annotation_layer_name,
            )
            annotation_layer.metadata["_name"] = annotation_layer_name
            ...
            annotation_layer.mouse_drag_callbacks.append(
                self.sam_octron_callbacks.on_mouse_press
            )
            annotation_layer.events.data.connect(
                self.sam_octron_callbacks.on_points_changed
            )
            logger.info(f"Created new mask + annotation layer '{layer_name}'")
...
def _apply_windows_taskbar_icon(viewer) -> None:
    ...
    try:
        ...
        qt_window = getattr(
            getattr(viewer, "window", None), "_qt_window", None
        )
...
def octron_gui():
    ...
    if _win:
        ...
        try:
            ...
            logger.debug(
                f"Could not resize napari docks after deferred show: {e}"
            )
